...
def launching_seq(vids_list=[], plylst ='', rate=1, cpu_nbr=8, f_frms=True, f_ftrs=True, f_mtchs=True, sample=False):
    """Function that spawns at maximum a number of processes equal to the parameter cpu_nbr.
    Use : pass either a list of vid ids or the name of playlist"""

    manager = Manager()
    shared_list = manager.list()

    # Generate vids paths from playlist
    if plylst:
        for el in os.listdir(pth_plylst(plylst)):
            vids_list.append(el)

    # Repeat until all videos have been processed
    target = iter0_seq
    while not len(vids_list) == 0:
        args_l=[]
        if len(vids_list) >= cpu_nbr:
            for i in range(cpu_nbr):
                args_iter0 = (pth_vid(v_id=vids_list.pop(), plylst=plylst), rate, f_frms, f_ftrs, f_mtchs, sample)
    ...
    # Finish until all sets have been processed
    target = sfm_pipe_seq
...
def iter0_seq(args_iter0, shared_list):
    ...
    sets = os.listdir(paths_sets)

    for pth in [os.path.join(paths_sets, set) for set in sets]:
...
def sfm_pipe_seq(pth_sets, width):
    ...
    for pth_set in pth_sets:
        args = (pth_set, width)
        current = Process(target=sfm_pipe, args=args)
        current.start()
        current.join()
