...
def path_to_vid_file(v_id, format):
    ...
    return os.path.join(path_to_vid_dir(v_id), vid_file)


def vid_xtrct(v_id, format,start=0, stop=30):
    ...
    split = path_to_vid_file(v_id=v_id, format=format).split(sep='.', maxsplit=1)

    path_to_trim_file = '{}-{}-{}.{}'.format(split[0], start, stop, split[1])

    cmd_str = 'avconv -i {} -s {} -t {} -codec copy {}'.format(path_to_vid_file(v_id=v_id, format=format),
                                                               start,
                                                               stop,
                                                               path_to_trim_file)
    os.system(cmd_str)
...
def frame_xtrct(v_id, format, imgs_per_sec=2):
    ...
    path_file = path_to_vid_file(v_id=v_id, format=format)
...
if __name__ == '__main__':
    ...
    timer.start()
    print('Vid extract')
    vid_xtrct(v_id=v_id, format='.webm', start=start, stop=stop)
    timer.stop()

    timer.start()
    print('frame extract')
    frame_xtrct(v_id=v_id, format='-00:00:10-00:00:40.webm') # TODO coder fonction pour le format
    timer.stop()
